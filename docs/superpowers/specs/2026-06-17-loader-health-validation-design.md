...
### 1. `src/preview/loader-health.ts` (NEW, pure / unit-testable)
...
### 2. `src/preview/sdk-mocks.ts` (MODIFY)
...
`tests/preview/loader-health.test.ts` (new):
...
- `src/preview/loader-health.ts` — NEW
- `src/preview/sdk-mocks.ts` — parameterize by `mraidMode`
- `src/preview/server.ts` — `?mraidMode=` wiring + emit loader-health checks
