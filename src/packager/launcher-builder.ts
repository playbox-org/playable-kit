 ...
 */
export const MOLOCO_V2_MACRO_SPEC: ReadonlyArray<{
  key: string
  placeholder: string
}> = [
  ...
  {
    key: 'taps_for_redirection',
    placeholder: '#PLAYABLE_TAPS_FOR_REDIRECTION#',
  },
  { key: 'cachebuster', placeholder: '#CACHEBUSTER#' },
  {
    key: 'draw_custom_close_button',
    placeholder: '#DRAW_CUSTOM_CLOSE_BUTTON#',
  },
]
...
export function buildLauncher(opts: LauncherBuildOptions): string {
  ...
  if (opts.includeSplash) {
    ...
    const hideJs =
      splash.hideJs + 'setTimeout(window.__plbx_splash_hide,12000);'
 ...
 */
export function fillLauncherPayloadUrl(
  launcherHtml: string,
  payloadUrl: string,
): string {
...
export function validateLauncher(launcherHtml: string): LauncherCheck[] {
  ...
  add(
    'asset_provider',
    'ASSET_PROVIDER metadata present',
    /<!--[\s\S]*?ASSET_PROVIDER=/.test(html),
    'metadata comment header missing ASSET_PROVIDER=',
  )

  add(
    'asset_version',
    `ASSET_VERSION=${MOLOCO_V2_ASSET_VERSION}`,
    new RegExp(
      `ASSET_VERSION=${MOLOCO_V2_ASSET_VERSION.replace('.', '\\.')}\\b`,
    ).test(html),
  ...
  add(
    'asset_revision',
    'ASSET_REVISION format YYYYMMDD.NN',
  ...
  add(
    'mraid_js',
    '<script src="mraid.js"> present',
    /<script\s+src=["']?mraid\.js["']?[^>]*>/i.test(html),
    '<script src="mraid.js"> missing',
  )

  add(
    'macros_declared',
    'MOLOCO_MACROS declared',
    /MOLOCO_MACROS\s*=/.test(html),
  ...
  for (const m of MOLOCO_V2_MACRO_SPEC) {
    ...
    const re = new RegExp(
      `["']?${m.key}["']?\\s*:\\s*["']${escapeRegExp(m.placeholder)}["']`,
    )
    if (!re.test(html)) wrongMacros.push(m.key)
  }
  add(
    'macro_values',
    'All macros use Moloco spec placeholders',
    wrongMacros.length === 0,
  ...
  add(
    'imp_beacon',
    '%{IMP_BEACON} present + last before </body>',
  ...
  add(
    'size',
    `Launcher < ${LAUNCHER_MAX_BYTES} B`,
    bytes <= LAUNCHER_MAX_BYTES,
