 ...
 *  passed value; otherwise the build-time kit version constant (bundler-safe —
 *  no package.json reads at runtime). */
function resolvePackagerVersion(explicit?: string): string {
  if (explicit) return explicit
  return KIT_VERSION
}

export async function packageForNetworks(
  ...
  const versionBanner = buildVersionBanner(
    resolvePackagerVersion(options.packagerVersion),
  )
  ...
  const headStoreUrls = Array.from(
    new Set(
      [
        options.config.storeUrlAndroid,
        options.config.storeUrlIos,
        ...extractStoreUrls(options.buildDir),
    ...
    try {
      ...
      if (network.format === 'launcher-payload') {
        options.onProgress?.(
          networkId,
          'processing',
          'Building launcher + payload...',
        )

        const lpConfig = network.launcherPayload
        if (!lpConfig) {
          throw new Error(
            `Network "${networkId}" has format=launcher-payload but no launcherPayload config`,
          )
        }
        ...
        const assetRevision =
          new Date().toISOString().slice(0, 10).replace(/-/g, '') + '.00'
        const launcher = buildLauncher({
          // Panel-configurable (Moloco CDN card); falls back to the network default.
          assetProvider:
            options.templateVariables?.assetProvider || lpConfig.assetProvider,
          assetTitle,
          assetRevision,
        ...
        if (structuralErrors.length > 0) {
          throw new Error(
            `[${network.name}] launcher.html structural errors: ${structuralErrors.join('; ')}`,
          )
        }
        ...
        const inlineScriptPayload = payloadJs.replace(
          /<\/script>/gi,
          '<\\/script>',
        )
      ...
      for (const format of formats) {
        ...
        const needsInlinedHtml =
          format === 'html' ||
          network.singleFileZip ||
          (format === 'zip' && network.dualFormat && network.inlineAssets)
        // Determine if the inlined HTML should be wrapped in a ZIP
        const wrapInZip =
          format === 'zip' &&
          (network.singleFileZip ||
            (network.dualFormat && network.inlineAssets))
        ...
        if (needsInlinedHtml) {
          ...
          options.onProgress?.(
            networkId,
            'processing',
            `Building ${wrapInZip ? 'single-file ZIP' : format.toUpperCase()}...`,
          )
 ...
 */
function assertHasRequiredStrings(
  html: string,
  required: string[],
  networkName: string,
): void {
...
function resolveInlinedEncodings(
  ...
  const sel: ('base64' | 'base122')[] =
    config.assetEncodings && config.assetEncodings.length
      ? config.assetEncodings
      : ['base64']
