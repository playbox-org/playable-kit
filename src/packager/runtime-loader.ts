...
import { JSZIP_MIN_JS } from '../generated/jszip-min'
 ...
 * Embedded at codegen time (src/generated/jszip-min.ts) — never resolved
 * from node_modules at runtime (bundler safety).
 */
 ...
 */
export function generateRuntimeLoader(
...
export function generateFullHtml(params: {
  ...
  const encoding: 'base64' | 'base122' =
    mode === 'self-contained' && params.encoding === 'base122'
      ? 'base122'
      : 'base64'
  ...
  const zipRawBytes =
    encoding === 'base122' ? Buffer.from(zipBase64, 'base64') : null
  const zipEncoded =
    encoding === 'base122'
      ? encodeBase122(new Uint8Array(zipRawBytes!))
      : zipBase64
  ...
  rewrittenHtml = rewrittenHtml.replace(scriptSrcRegex, (match, src) => {
    ...
    if (buildDir) {
      try {
        ...
        /* file not in build dir — load from ZIP */
      }
    }
  ...
  if (mode === 'self-contained') {
    ...
    if (encoding === 'base122') {
      ...
      injection +=
        '<script>' +
        emitBase122Decoder() +
        'window.__plbx_enc="b122";window.__plbx_zip_len=' +
        zipRawBytes!.length +
        ';</script>\n'
    }
    ...
    if (jsModules && Object.keys(jsModules).length > 0) {
      injection +=
        '<script>window.__res = ' + JSON.stringify(jsModules) + ';</script>\n'
    } else {
      injection += '<script>window.__res = {};</script>\n'
    }
    // Script execution order for boot sequence
    injection +=
      '<script>window.__plbx_scripts = ' +
  ...
  if (bodyCloseIndex !== -1) {
    return (
      rewrittenHtml.slice(0, bodyCloseIndex) +
      '\n' +
      injection +
      '\n' +
  ...
  if (htmlCloseIndex !== -1) {
    return (
      rewrittenHtml.slice(0, htmlCloseIndex) +
      '\n' +
      injection +
      '\n' +
