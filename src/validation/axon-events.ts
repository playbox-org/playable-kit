...
export function validateAxonEvents(usage: AxonUsage): AxonCheck[] {
  ...
  checks.push({
    ...
    level: 'warn',
    detail:
      'DISPLAYED is the only mandatory Axon event — fire it when the creative is ready for interaction.',
  })
  ...
  if (set.has('LOADING') || set.has('LOADED')) {
    checks.push({
      ...
      level: 'warn',
      detail:
        'LOADING and LOADED are a pair — fire both (LOADING → LOADED → DISPLAYED) or neither.',
    })
  }
...
const isChallengeEvent = (name: string): boolean =>
  name.indexOf('CHALLENGE_') === 0
 ...
 */
export function validateAxonSequence(
  sequence: string[],
  timestamps?: number[],
): AxonCheck[] {
  ...
  if (timestamps && timestamps.length === sequence.length) {
    ...
    for (let i = 1; i < challengeFires.length; i++) {
      ...
      if (dt < MIN_CHALLENGE_INTERVAL_MS) {
        tooClose.push(
          `${challengeFires[i - 1].name}→${challengeFires[i].name} ${Math.round(dt)}ms`,
        )
      }
    }
  ...
  checks.unshift({
    ...
    label:
      failures.length === 0
        ? 'All events conform to spec'
        : `${failures.length} spec issue(s)`,
...
function redefinitionCheck(redefines: boolean): AxonCheck {
  return {
    ...
    level: 'warn',
    detail:
      'Do not define window.ALPlayableAnalytics — the AppLovin SDK provides it automatically.',
  }
}
