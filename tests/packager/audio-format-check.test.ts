...
} from '../../src/validation/audio-format-check'
...
describe('detectRiskyAudio', () => {
  ...
  it('exposes the risky extension set (ogg/opus/webm)', () => {
    expect(RISKY_AUDIO_EXTENSIONS).toEqual(
      expect.arrayContaining(['.ogg', '.opus', '.webm']),
    )
...
describe('isHostileMp3 / detectHostileMp3', () => {
  ...
  it('finds hostile mp3s in a build tree, only the hostile one', () => {
    ...
    expect(found.some((p) => p.endsWith('webkit-hostile-vbr.fixed.mp3'))).toBe(
      false,
    )
  })
