...
import {
  ...
  AXON_EVENTS,
} from '../../src/validation/axon-events'
...
describe('extractAxonUsage', () => {
  ...
  it('captures non-spec (custom/typo) event names so they can be flagged', () => {
    ...
    write(
      dir,
      'a.js',
      "trackEvent('CHALLANGE_SOLVED'); trackEvent('myCustomEvent');",
    )
    expect(extractAxonUsage(dir).events).toEqual([
      'CHALLANGE_SOLVED',
      'myCustomEvent',
  ...
  it('flags redefinesAnalytics when source assigns window.ALPlayableAnalytics', () => {
    ...
    write(
      dir,
      'a.js',
      'window.ALPlayableAnalytics = { trackEvent: function(){} };',
    )
  ...
  it('returns empty usage and does not throw for a missing directory', () => {
    ...
    expect(extractAxonUsage(missing)).toEqual({
      events: [],
...
describe('validateAxonEvents', () => {
  const failed = (checks: ReturnType<typeof validateAxonEvents>) =>
  ...
  it('errors on an unknown/typo event name', () => {
    const checks = validateAxonEvents({
      events: ['DISPLAYED', 'CHALLANGE_SOLVED'],
      redefinesAnalytics: false,
    })
  ...
  it('passes the LOADING/LOADED pair rule when both are present', () => {
    const checks = validateAxonEvents({
      events: ['DISPLAYED', 'LOADING', 'LOADED'],
      redefinesAnalytics: false,
    })
  ...
  it('does not raise the LOADING/LOADED pair rule when neither is present', () => {
    ...
    expect(
      checks.find((c) => c.id === 'loaded_requires_loading'),
    ).toBeUndefined()
  })

  it('warns when CHALLENGE_STARTED has no completion event', () => {
    const checks = validateAxonEvents({
      events: ['DISPLAYED', 'CHALLENGE_STARTED'],
      redefinesAnalytics: false,
    })
...
describe('validateAxonSequence', () => {
  const byId = (checks: ReturnType<typeof validateAxonSequence>, id: string) =>
    checks.find((c) => c.id === id)
  const failed = (checks: ReturnType<typeof validateAxonSequence>) =>
    checks.filter((c) => !c.ok).map((c) => c.id)
