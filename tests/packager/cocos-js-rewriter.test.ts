...
} from '../../src/packager/cocos-js-rewriter'
...
describe('shouldRewriteCocosJs', () => {
  ...
  it('rejects non-js inside cocos-js', () => {
    expect(
      shouldRewriteCocosJs('cocos-js/assets/spine.js.mem-DkFYcHIO.bin'),
    ).toBe(false)
  })
})
...
describe('rewriteCocosJs', () => {
  ...
  it('replaces document.currentScript reads', () => {
    ...
    expect(out).toContain(
      '(window._PLBX_currentScript||document.currentScript).src',
    )
    expect(out).not.toMatch(
      /(?<!_PLBX_currentScript\|\|)document\.currentScript\.src/,
    )
  })
