...
} from '../../src/packager/launcher-builder'
import { join } from 'path'
import { mkdirSync, writeFileSync, existsSync, rmSync, readFileSync } from 'fs'
import vm from 'vm'
import { MolocoV2Adapter } from '../../src/packager/network-adapters/moloco-v2'
...
afterAll(() => {
  if (existsSync(MOCK_BUILD))
    rmSync(MOCK_BUILD, { recursive: true, force: true })
...
describe('MolocoV2 target', () => {
  ...
  it('honors templateVariables.assetProvider / assetTitle in the launcher metadata', async () => {
    await packageForNetworks({
      ...
      config: defaultConfig,
      templateVariables: {
        assetProvider: 'MyStudio',
        assetTitle: 'My Great Game',
      },
    })
    const launcher = readFileSync(
      join(PACK_OUTPUT, 'molocoV2', 'launcher.html'),
      'utf-8',
    )
  ...
  it('emits self-contained launcher-local.html with inlined payload for QA', async () => {
    ...
    const localPath = launcherPath.replace(
      /launcher\.html$/,
      'launcher-local.html',
    )
    ...
    const inlineMatch = local.match(
      /<script>((?:(?!<\/script>)[\s\S])*)<\/script>\s*%\{IMP_BEACON\}/,
    )
  ...
  it('fillLauncherPayloadUrl substitutes placeholder with real CDN URL', () => {
    ...
    const replaced = fillLauncherPayloadUrl(
      launcher,
      'https://cdn.moloco.com/abc/payload.js',
    )
...
describe('validateLauncher (spec compliance gate)', () => {
  ...
  it('flags a non-spec macro value (the v0.2.4 regression)', () => {
    const bad = buildLauncher(baseOpts).replace(
      '#CLICK_TEMPLATE_ESC#',
      '#CLICK#',
    )
  ...
  it('accepts the #PAYLOAD_URL# placeholder and an absolute payload URL', () => {
    ...
    const filled = fillLauncherPayloadUrl(
      buildLauncher(baseOpts),
      'https://cdn-f.adsmoloco.com/x/p.js',
    )
  ...
  describe('asset metadata sanitization', () => {
    ...
    it('keeps letters, digits, space, dash, underscore, dot untouched', () => {
      const launcher = buildLauncher({
        ...baseOpts,
        assetTitle: 'piggy-merge_v2.0 RU',
      })
  ...
  describe('size gate accounts for the real CDN URL (launcher-final regression)', () => {
    ...
    it('exports a reserve at least as long as a real Moloco CDN asset URL', () => {
      const real =
        'https://cdn-f.adsmoloco.com/lkzEvERvJWoAD5Io/external/mq7w2jd2_atzkfgu_ol5h5sbyftowwphk.js'
    ...
    it('effectiveLauncherBytes adds the URL reserve while the placeholder is present', () => {
      ...
      expect(effectiveLauncherBytes(launcher)).toBe(
        raw - '#PAYLOAD_URL#'.length + PAYLOAD_URL_RESERVE_BYTES,
      )
      ...
      expect(effectiveLauncherBytes(filled)).toBe(
        Buffer.byteLength(filled, 'utf-8'),
      )
    })
    ...
    it('validateLauncher fails size when placeholder + reserve exceed the ceiling', () => {
      ...
      const padded = launcher.replace(
        '<body>',
        `<body><!--${'x'.repeat(pad - 7)}-->`,
      )
      expect(Buffer.byteLength(padded, 'utf-8')).toBeLessThanOrEqual(
        LAUNCHER_MAX_BYTES,
      )
    ...
    it('a compact-splash launcher with a long title fits the ceiling WITH the URL reserve', () => {
      ...
      expect(effectiveLauncherBytes(launcher)).toBeLessThanOrEqual(
        LAUNCHER_MAX_BYTES,
      )
...
describe('molocoV2 bridge — CTA fire & final_url fallback (§2.4)', () => {
  ...
  it('fires the CTA once per click even when download() is called twice (double-fire collapse)', () => {
    const { win, opens, beacons } = runBridge({
