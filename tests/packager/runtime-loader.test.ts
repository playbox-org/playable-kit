...
import {
  ...
  getJSZipRuntime,
  generateFullHtml,
} from '../../src/packager/runtime-loader'
...
describe('generateRuntimeLoader — shared invariants (both modes)', () => {
  for (const mode of ['self-contained', 'systemjs'] as const) {
    ...
    it(`[${mode}] toggles DEBUG flag`, () => {
      expect(generateRuntimeLoader({ mode, debug: true })).toContain(
        'DEBUG = true',
      )
...
describe('legacy loader (mode: systemjs)', () => {
  ...
  it('Image src defineProperty is configurable', () => {
    const code = generateRuntimeLoader({ mode: 'systemjs' })
    const imgBlock = code.slice(
      code.indexOf("Object.defineProperty(img, 'src'"),
    )
  ...
  for (const mode of ['self-contained', 'systemjs'] as const) {
    it(`[${mode}] runtime loader has no forbidden strings`, () => {
      ...
      for (const needle of FORBIDDEN)
        expect(code, `leaked "${needle}"`).not.toContain(needle)
    })
...
describe('generateFullHtml', () => {
  ...
  it('showSplash:true injects splash overlay + style + first-frame hook', () => {
    ...
    expect(result.indexOf('<div id="s">')).toBeLessThan(
      result.indexOf('__plbx_zip'),
    )
  })
