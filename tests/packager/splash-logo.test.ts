...
import { resolveSplashLogoDataUrl } from '../../src/packager/packager'
...
describe('resolveSplashLogoDataUrl', () => {
  ...
  it('returns undefined for an unsupported extension', () => {
    expect(
      resolveSplashLogoDataUrl(join(FIXTURES, 'fake-script.ts')),
    ).toBeUndefined()
  })

  it('returns undefined for a missing file', () => {
    expect(
      resolveSplashLogoDataUrl(join(FIXTURES, 'does-not-exist.png')),
    ).toBeUndefined()
  })
