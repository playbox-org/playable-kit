...
import {
  ...
  validateTemplate,
  TemplateContext,
} from '../../src/packager/template-resolver'
...
describe('resolveTemplate', () => {
  it('resolves system variables only', () => {
    expect(resolveTemplate('{networkId}/index.{ext}', baseCtx)).toBe(
      'applovin/index.html',
    )
  })
  ...
  it('resolves user-defined variables', () => {
    ...
    expect(resolveTemplate('{networkId}/{version}/{lang}.{ext}', ctx)).toBe(
      'applovin/2/en.html',
    )
  })

  it('leaves unknown tokens untouched', () => {
    expect(resolveTemplate('{networkId}/{unknown}.{ext}', baseCtx)).toBe(
      'applovin/{unknown}.html',
    )
  })
  ...
  it('default template produces backward-compatible output', () => {
    ...
    expect(resolveTemplate(defaultTemplate, baseCtx)).toBe(
      'applovin/index.html',
    )
  ...
  it('mixed casing in one template', () => {
    ...
    expect(resolveTemplate('{networkId}/TRM_{Network}.{ext}', ctx)).toBe(
      'ironsource/TRM_Ironsource.html',
    )
  })

  it('user-defined vars also support casing', () => {
    const ctx: TemplateContext = { ...baseCtx, project: 'myGame' }
    expect(resolveTemplate('{Project}_{network}.{ext}', ctx)).toBe(
      'Mygame_applovin.html',
    )
  })
})
...
describe('extractVariables', () => {
  ...
  it('normalizes casing: {Network} and {network} are the same var', () => {
    expect(extractVariables('{Network}/{network}.{ext}')).toEqual([
      'network',
      'ext',
    ])
  })
...
describe('getUserVariables', () => {
  it('filters out system vars', () => {
    expect(getUserVariables('{networkId}/{version}/{lang}.{ext}')).toEqual([
      'version',
      'lang',
    ])
  })
