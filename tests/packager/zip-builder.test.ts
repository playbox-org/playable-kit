...
import { buildZip } from '../../src/packager/zip-builder'
...
afterAll(() => {
  if (existsSync(ZIP_SOURCE))
    rmSync(ZIP_SOURCE, { recursive: true, force: true })
  if (existsSync(ZIP_OUTPUT))
    rmSync(ZIP_OUTPUT, { recursive: true, force: true })
})
...
describe('buildZip', () => {
  ...
  it('should add prefix to paths', async () => {
    ...
    const zip = await JSZip.loadAsync(readFileSync(result.outputPath))
    expect(Object.keys(zip.files).some((f) => f.startsWith('mintegral/'))).toBe(
      true,
    )
  })
  ...
  it('should add extra files like config.json', async () => {
    const result = await buildZip({
      ...
      extraFiles: [
        { zipPath: 'config.json', content: '{"playable_orientation": 0}' },
      ],
    })
    const zip = await JSZip.loadAsync(readFileSync(result.outputPath))
