...
describe('MolocoV2 macro reverse-lookup algorithm', () => {
  ...
  function keyForUrl(lookup: LookupMap, url: string): string | null {
    ...
    for (const raw of Object.keys(lookup)) {
      if (raw && url.length > raw.length && url.endsWith(raw))
        return lookup[raw]
    }
    return null
  }
  ...
  beforeEach(() => {
    lookup = buildLookup({
      ...
      final_url:
        'https%3A%2F%2Fplay.google.com%2Fstore%2Fapps%2Fdetails%3Fid%3Dcom.app',
      engagement: '',
    })
  })

  it('matches encoded URL to its macro key', () => {
    expect(keyForUrl(lookup, 'https%3A%2F%2Fdsp.moloco%2Fbeacon%2Fmv')).toBe(
      'mraid_viewable',
    )
  })

  it('matches decoded URL form to the same macro key', () => {
    expect(keyForUrl(lookup, 'https://dsp.moloco/beacon/mv')).toBe(
      'mraid_viewable',
    )
  })
  ...
  it('matches absolutized URL (extension re-wrap turns placeholder into fragment)', () => {
    ...
    const frag = buildLookup({
      mraid_viewable: '#IMP_TRACE_MRAID_VIEWABLE_ESC#',
    })
    expect(
      keyForUrl(
        frag,
    ...
    expect(keyForUrl(frag, '#IMP_TRACE_MRAID_VIEWABLE_ESC#')).toBe(
      'mraid_viewable',
    )
  })
